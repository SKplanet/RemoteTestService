...
WinAES::WinAES( const char* lpszContainer, int nFlags )
...
m_nFlags(nFlags), m_wszContainer(NULL), m_nIndex(INVALID_INDEX)
{
    if( NULL == lpszContainer ) {
        lpszContainer = "Temporary - OK to Delete";
        m_nFlags |= DELETE_CONTAINER;
    }

    size_t len = strlen( lpszContainer ) + 1;
    assert( 0 != len );

    if( len ) {
        m_wszContainer = new char[ len ];
        assert( NULL != m_wszContainer );

        errno_t err = 0;
        err = strcpy_s( m_wszContainer, len, lpszContainer );
        assert( 0 == err );
    }
...
bool WinAES::AcquireContext( const char* lpszContainer )
{
...
bool WinAES::SetKey( const byte* key, int ksize )
{
    ...
    try
    {
        ...
        if(!CryptSetKeyParam(m_hAesKey, KP_PADDING, (BYTE*)&dwMode, 0)) {
            assert( false );
            throw WinAESException("SetKey: Set PKCS5_PADDING mode failed");
        }
...
bool WinAES::Decrypt( byte* buffer, /*In*/size_t bsize, /*In*/size_t csize, /*Out*/ size_t& psize )
{
    ...
    try
    {
        ...
        // Temporary for API
        DWORD d = (DWORD)csize;
        result = CryptDecrypt( m_hAesKey, NULL, TRUE, 0 /*CRYPT_DECRYPT_RSA_NO_PADDING_CHECK*/, buffer, &d );
        //assert( TRUE == result );
        if( FALSE == result ) {
            // Build a useful message
            ostringstream emessage;
